...
/// `Biquad32<16, 20>` will create a 32-bit biquad filter for a 32-bit signal with 16 fractional bits on the
...
///
/// While you can create Biqad filters by passing raw coefficients using the `Biquad32::new()` method,
...
/// let mut filter = Biquad32::<20, 20>::lowpass(ts, f0, q);
///
...
pub struct Biquad32<const X: i32, const Y: i32> {
    // coefficients
    a1: i32,
    a2: i32,
    b0: i32,
...
impl<const X: i32, const Y: i32> Biquad32<X, Y> {
    ...
    /// let ts = 1./10e3;  // sampling period
    /// let f0 = 15.0;  // cutoff frequency
    /// let mut filter = Biquad32::<25, 25>::single_pole_lowpass(ts, f0);
    /// ```
    ...
    /// let f0 = 15.0;  // cutoff frequency
    /// let q = core::f32::consts::FRAC_1_SQRT_2; // butterworth
    /// let mut filter = Biquad32::<20, 20>::highpass(ts, f0, q);
    /// ```
    ...
    /// let q = core::f32::consts::FRAC_1_SQRT_2; // butterworth
    /// let mut filter = Biquad32::<20, 20>::bandpass(ts, f0, q);
    /// ```
    ...
    /// let q = core::f32::consts::FRAC_1_SQRT_2; // butterworth
    /// let mut filter = Biquad32::<20, 20>::notch(ts, f0, q);
    /// ```
    ...
    /// let kp = 0.2;
    /// let ki = 0.1;
    /// let mut filter = Biquad32::<25, 30>::pi(kp, ki);
    /// ```
...
mod tests {
    ...
    fn pi() {
        let kp = 0.2;
        let ki = 0.1;
        let mut filter = Biquad32::<25, 25>::pi(kp, ki);
    ...
    fn lowpass() {
        ...
        let f0 = 60.0;
        let q = core::f32::consts::FRAC_1_SQRT_2; // butterworth
        let mut filter = Biquad32::<25, 25>::lowpass(1. / fs, f0, q);
    ...
    fn highpass() {
        ...
        let f0 = 60.0;
        let q = core::f32::consts::FRAC_1_SQRT_2; // butterworth
        let mut filter = Biquad32::<25, 25>::highpass(1. / fs, f0, q);
    ...
    fn bandpass() {
        ...
        let f0 = 60.0;
        let q = core::f32::consts::FRAC_1_SQRT_2; // butterworth
        let mut filter = Biquad32::<25, 25>::bandpass(1. / fs, f0, q);
    ...
    fn notch() {
        ...
        let f0 = 60.0;
        let q = core::f32::consts::FRAC_1_SQRT_2; // butterworth
        let mut filter = Biquad32::<25, 25>::notch(1. / fs, f0, q);
    ...
    fn limits() {
        let kp = 0.2;
        let ki = 0.1;
        let mut filter = Biquad32::<25, 25>::pi(kp, ki).with_max(1.0);
        ...
        let mut filter = Biquad32::<25, 25>::pi(kp, ki).with_min(-1.0);
        let mut y = FixedI32::<25>::ZERO;
        for _ in 0..100 {
            y = filter.update(FixedI32::<25>::from_num(-1.0))
        }
    ...
    fn scaling() {
        ...
        let f0 = 60.0;
        let q = core::f32::consts::FRAC_1_SQRT_2; // butterworth
        let mut filter0 = Biquad32::<25, 25>::lowpass(1. / fs, f0, q);
        let mut filter1 = Biquad32::<25, 20>::lowpass(1. / fs, f0, q);
        let mut filter2 = Biquad32::<20, 25>::lowpass(1. / fs, f0, q);
